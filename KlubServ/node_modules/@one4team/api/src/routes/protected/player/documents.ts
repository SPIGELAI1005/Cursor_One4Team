...
// Mock documents data - replace with actual database queries
const mockDocuments = [
  {
    id: 'doc_1',
    name: 'Player Card',
...
// GET /api/player/documents - Get player documents
router.get('/', (req, res) => {
  try {
    ...
    // Mock: verify player can only access their own documents
...
// GET /api/player/documents/card - Get player card details
router.get('/card', (req, res) => {
  try {
    ...
    // Mock: verify player can only access their own player card
    if (playerId !== 'player_123') {
      return res.status(403).json({ error: 'Access denied' });
    }
...
// GET /api/player/documents/:id - Get specific document details
...
// GET /api/player/documents/:id/download - Download document
router.get('/:id/download', (req, res) => {
  try {
    ...
    // Mock: verify player can only download their own documents
    if (playerId !== 'player_123') {
      return res.status(403).json({ error: 'Access denied' });
    }
    ...
    // Mock download response - in real implementation, serve actual file
    res.json({
      success: true,
      data: {
